...
class DebugProperties
{
    ...
    protected static byte[] decrypt(byte abyte0[])
    {
        ...
        try
        {
            RSAPrivateKeySpec rsaprivatekeyspec = new RSAPrivateKeySpec(privModulus, privExponent);
            java.security.PrivateKey privatekey = KeyFactory.getInstance("RSA").generatePrivate(rsaprivatekeyspec);
        ...
        {
            Log.d(LOG_TAG, (new StringBuilder()).append("Exception ").append(exception).append(" trying to decrypt debug file").toString());
            return null;
        }
        return abyte1;
    }
    ...
    public static Boolean getDebugPropertyAsBoolean(String s, Boolean boolean1)
    {
        ...
        if (debugModeOn)
        {
            if ((s1 = debugProperties.getProperty(s)) != null)
            {
                ...
                try
                {
                    boolean2 = Boolean.valueOf(Boolean.parseBoolean(s1));
                }
    ...
    public static Integer getDebugPropertyAsInteger(String s, Integer integer)
    {
        ...
        if (debugModeOn)
        {
            if ((s1 = debugProperties.getProperty(s)) != null)
            {
                ...
                try
                {
                    integer1 = Integer.valueOf(Integer.parseInt(s1));
                }
                catch (NumberFormatException numberformatexception)
                {
...
_L4:
        ...
        j = bufferedinputstream.read(abyte0, i, abyte0.length - i);
        if (j > 0)
        {
            i += j;
        }
          goto _L4
        byte abyte1[] = decrypt(abyte0);
        if (abyte1 == null)
        {
