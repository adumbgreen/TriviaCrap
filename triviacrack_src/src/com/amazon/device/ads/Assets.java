...
//            Log, Base64, Settings, NumberUtils, 
...
class Assets
{
    ...
    private void writeAssetToDisk(CircuitBreaker circuitbreaker, byte abyte0[], String s)
    {
        ...
        try
        {
            outputstream = createOutputStream(s);
        }
        catch (IOException ioexception)
        {
            Log.e("Assets", "Could not write an asset to disk: %s", new Object[] {
                s
            });
        ...
        writeBytesToOutputStream(bufferedoutputstream, abyte0);
        try
        {
        ...
        ioexception2;
        Log.w("Assets", "Caught IOException while writing asset to disk: %s", new Object[] {
            s
        });
        circuitbreaker.breakCircuit();
        try
    ...
    public boolean ensureAssetsCreated()
    {
        ...
        if (baseFilePath != null) goto _L2; else goto _L1
_L1:
        String s;
        baseFilePath = getBaseFilePath();
        s = baseFilePath;
...
_L2:
        ...
        i = settings.getInt("mraid-assets-version", 0);
        boolean flag = false;
        if (i >= 5) goto _L5; else goto _L4
_L4:
        Object aobj[] = new Object[1];
        aobj[0] = Integer.valueOf(5);
        Log.d("Assets", "Writing assets to disk: version %d", aobj);
        if (!writeAssetsToDisk()) goto _L7; else goto _L6
_L6:
        settings.putInt("mraid-assets-version", 5);
        flag = true;
_L5:
        long l1;
        MetricsCollector metricscollector;
        l1 = NumberUtils.convertToMillisecondsFromNanoseconds(System.nanoTime() - l);
        metricscollector = Metrics.getInstance().getMetricsCollector();
        if (!flag)
        {
            break MISSING_BLOCK_LABEL_171;
        }
        Metrics.MetricType metrictype = Metrics.MetricType.ASSETS_CREATED_LATENCY;
_L8:
        metricscollector.publishMetricInMilliseconds(metrictype, l1);
        flag1 = true;
          goto _L3
_L7:
        ...
        exception;
        throw exception;
        metrictype = Metrics.MetricType.ASSETS_ENSURED_LATENCY;
          goto _L8
    }
    ...
    protected String getBaseFilePath()
    {
        ...
        {
            return (new StringBuilder()).append(InternalAdRegistration.getInstance().getFilesDir().getAbsolutePath()).append(File.separator).toString();
        } else
        {
            return null;
        }
    ...
    {
        return (new StringBuilder()).append(baseFilePath).append(s).toString();
    }

    protected void writeBytesToOutputStream(OutputStream outputstream, byte abyte0[])
    {
