// Decompiled by Jad v1.5.8e. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://www.geocities.com/kpdus/jad.html
// Decompiler options: braces fieldsfirst space lnc 

package com.mdotm.android.view;

import android.app.Activity;
import android.content.Context;
import android.content.Intent;
import android.content.res.Configuration;
import android.content.res.Resources;
import android.graphics.Bitmap;
import android.graphics.BitmapFactory;
import android.net.Uri;
import android.os.Bundle;
import android.os.Handler;
import android.util.DisplayMetrics;
import android.view.KeyEvent;
import android.view.MotionEvent;
import android.view.View;
import android.webkit.WebSettings;
import android.webkit.WebView;
import android.webkit.WebViewClient;
import android.widget.ImageView;
import android.widget.RelativeLayout;
import com.mdotm.android.c.a;
import com.mdotm.android.c.c;
import com.mdotm.android.c.d;
import com.mdotm.android.d.b;
import com.mdotm.android.e.f;

// Referenced classes of package com.mdotm.android.view:
//            c, InterstitialImageView, a

public class MdotMActivity extends Activity
    implements c, d
{

    Handler a;
    boolean b;
    boolean c;
    private b d;
    private a e;
    private long f;
    private com.mdotm.android.view.c g;
    private InterstitialImageView h;
    private com.mdotm.android.view.a i;
    private WebView j;
    private int k;
    private boolean l;
    private RelativeLayout m;
    private int n;
    private boolean o;
    private int p;
    private int q;

    public MdotMActivity()
    {
        j = null;
        b = false;
        c = false;
    }

    private int a(int i1)
    {
        float f1 = getResources().getDisplayMetrics().density;
        com.mdotm.android.e.d.b(this, (new StringBuilder("scale = ")).append(f1).toString());
        int j1 = (int)(0.5F + f1 * (float)i1);
        com.mdotm.android.e.d.b(this, (new StringBuilder("result px = ")).append(j1).toString());
        return j1;
    }

    static WebView a(MdotMActivity mdotmactivity)
    {
        return mdotmactivity.j;
    }

    static void a(MdotMActivity mdotmactivity, boolean flag)
    {
        mdotmactivity.l = flag;
    }

    static a b(MdotMActivity mdotmactivity)
    {
        return mdotmactivity.e;
    }

    static boolean c(MdotMActivity mdotmactivity)
    {
        return mdotmactivity.l;
    }

    static b d(MdotMActivity mdotmactivity)
    {
        return mdotmactivity.d;
    }

    private RelativeLayout g()
    {
        RelativeLayout relativelayout = new RelativeLayout(this);
        relativelayout.setLayoutParams(new android.widget.RelativeLayout.LayoutParams(-2, -2));
        RelativeLayout relativelayout1 = new RelativeLayout(this);
        android.widget.RelativeLayout.LayoutParams layoutparams = new android.widget.RelativeLayout.LayoutParams(-1, -1);
        layoutparams.addRule(3, 11);
        relativelayout1.setLayoutParams(layoutparams);
        relativelayout1.setGravity(17);
        relativelayout1.setId(2);
        relativelayout1.setBackgroundColor(0);
        relativelayout1.setOnTouchListener(new android.view.View.OnTouchListener() {

            final MdotMActivity a;

            public boolean onTouch(View view, MotionEvent motionevent)
            {
                if (motionevent.getAction() == 1 && !com.mdotm.android.view.MdotMActivity.c(a))
                {
                    com.mdotm.android.view.MdotMActivity.a(a, true);
                    a.a();
                }
                return true;
            }

            
            {
                a = MdotMActivity.this;
                super();
            }
        });
        android.widget.RelativeLayout.LayoutParams layoutparams1 = new android.widget.RelativeLayout.LayoutParams(-2, -2);
        layoutparams1.addRule(13);
        g.setLayoutParams(layoutparams1);
        relativelayout.addView(g);
        relativelayout.addView(h());
        relativelayout.addView(relativelayout1);
        return relativelayout;
    }

    private RelativeLayout h()
    {
        RelativeLayout relativelayout = new RelativeLayout(this);
        android.widget.RelativeLayout.LayoutParams layoutparams = new android.widget.RelativeLayout.LayoutParams(-2, -2);
        layoutparams.addRule(10);
        layoutparams.setMargins(0, 10, 10, 0);
        relativelayout.setId(11);
        relativelayout.setLayoutParams(layoutparams);
        relativelayout.setBackgroundColor(0);
        ImageView imageview = new ImageView(this);
        android.widget.RelativeLayout.LayoutParams layoutparams1;
        if (o)
        {
            imageview.setImageResource(q);
        } else
        {
            imageview.setImageBitmap(a(""));
        }
        layoutparams1 = new android.widget.RelativeLayout.LayoutParams(a(25), a(25));
        layoutparams1.addRule(11);
        imageview.setLayoutParams(layoutparams1);
        imageview.setOnClickListener(new android.view.View.OnClickListener() {

            final MdotMActivity a;

            public void onClick(View view)
            {
                a.onDismissScreen();
            }

            
            {
                a = MdotMActivity.this;
                super();
            }
        });
        relativelayout.addView(imageview);
        return relativelayout;
    }

    public Bitmap a(String s)
    {
        Bitmap bitmap;
        try
        {
            byte abyte0[] = com.mdotm.android.e.c.a(s, 0);
            bitmap = BitmapFactory.decodeByteArray(abyte0, 0, abyte0.length);
        }
        catch (Exception exception)
        {
            exception.getMessage();
            return null;
        }
        return bitmap;
    }

    void a()
    {
        com.mdotm.android.e.d.b(this, "  Selected to clicked  ");
        if (d.e() != null)
        {
            if (!b)
            {
                b = true;
                (new Thread(d.e()) {

                    final MdotMActivity a;
                    private final String b;

                    static MdotMActivity a(_cls4 _pcls4)
                    {
                        return _pcls4.a;
                    }

                    public void run()
                    {
                        a.c();
                        com.mdotm.android.e.d.b(this, (new StringBuilder("Launch type is ")).append(com.mdotm.android.view.MdotMActivity.d(a).k()).toString());
                        if (com.mdotm.android.view.MdotMActivity.d(a).k() == 2)
                        {
                            Intent intent = new Intent("android.intent.action.VIEW", Uri.parse(b));
                            intent.addFlags(0x10000000);
                            try
                            {
                                a.startActivity(intent);
                            }
                            catch (Exception exception)
                            {
                                com.mdotm.android.e.d.b(this, (new StringBuilder("Could not open browser on ad click to ")).append(exception).toString());
                            }
                            a.b = false;
                            if (com.mdotm.android.view.MdotMActivity.b(a) != null)
                            {
                                com.mdotm.android.view.MdotMActivity.b(a).b();
                            }
                            return;
                        } else
                        {
                            a.a.post(new Runnable(this, b) {

                                final _cls4 a;
                                private final String b;

                                public void run()
                                {
                                    com.mdotm.android.view.MdotMActivity.a(com.mdotm.android.view._cls4.a(a)).loadUrl(b);
                                }

            
            {
                a = _pcls4;
                b = s;
                super();
            }
                            });
                            return;
                        }
                    }

            
            {
                a = MdotMActivity.this;
                b = s;
                super();
            }
                }).start();
                return;
            } else
            {
                com.mdotm.android.e.d.b(this, "ad selection under progress");
                return;
            }
        } else
        {
            com.mdotm.android.e.d.a(this, "selected ads landing url is null");
            return;
        }
    }

    public void b()
    {
        com.mdotm.android.e.d.a(this, "Error while loading the ad");
        onDismissScreen();
    }

    public void c()
    {
        l = true;
        com.mdotm.android.e.d.b(this, "clicked on ad");
        if (e != null)
        {
            e.d();
        }
        if (g != null)
        {
            g.a(a);
        }
    }

    public void d()
    {
        if (e != null)
        {
            e.b();
        }
    }

    public void didShowInterstitial()
    {
    }

    public void e()
    {
        com.mdotm.android.e.d.b(this, "completed palying video");
    }

    public void f()
    {
        com.mdotm.android.e.d.b(this, "Error while palying video");
        if (e != null)
        {
            e.c();
        }
        onDismissScreen();
    }

    public void onBannerAdClick()
    {
    }

    public void onConfigurationChanged(Configuration configuration)
    {
        super.onConfigurationChanged(configuration);
    }

    protected void onCreate(Bundle bundle)
    {
        com.mdotm.android.e.d.b(this, "onCreate is called");
        super.onCreate(bundle);
        n = getIntent().getIntExtra("style", 0);
        o = getIntent().getBooleanExtra("frame", false);
        p = getIntent().getIntExtra("frame_id", 0);
        q = getIntent().getIntExtra("close_btn_id", 0);
        if (o)
        {
            n = 1;
        }
        a = new Handler();
        j = new WebView(this);
        j.getSettings().setJavaScriptEnabled(true);
        j.setWebViewClient(new WebViewClient() {

            boolean a;
            Runnable b;
            final MdotMActivity c;

            static MdotMActivity a(_cls1 _pcls1)
            {
                return _pcls1.c;
            }

            public void onPageFinished(WebView webview, String s)
            {
                super.onPageFinished(webview, s);
                a = false;
                com.mdotm.android.e.d.b(this, (new StringBuilder("Click page finished ")).append(s).toString());
                c.b = false;
            }

            public void onPageStarted(WebView webview, String s, Bitmap bitmap)
            {
                com.mdotm.android.e.d.b(this, (new StringBuilder("started Loading url ")).append(s).toString());
                if (c.b)
                {
                    a = true;
                    c.a.removeCallbacks(b);
                    c.a.postDelayed(b, 15000L);
                }
                super.onPageStarted(webview, s, bitmap);
            }

            public void onReceivedError(WebView webview, int i1, String s, String s1)
            {
                super.onReceivedError(webview, i1, s, s1);
                a = false;
                if (c.b)
                {
                    com.mdotm.android.e.d.a(this, (new StringBuilder("Unable to load landing url : ")).append(s).toString());
                }
                c.b = false;
            }

            public boolean shouldOverrideUrlLoading(WebView webview, String s)
            {
                com.mdotm.android.e.d.b(this, (new StringBuilder("returned url ")).append(s).toString());
                if (s != null && s.startsWith("market://"))
                {
                    try
                    {
                        Intent intent = new Intent("android.intent.action.VIEW", Uri.parse(s));
                        webview.getContext().startActivity(intent);
                        if (com.mdotm.android.view.MdotMActivity.b(c) != null)
                        {
                            com.mdotm.android.view.MdotMActivity.b(c).b();
                        }
                    }
                    catch (Exception exception)
                    {
                        com.mdotm.android.e.d.a(this, "Google Play store app is not installed");
                        exception.printStackTrace();
                    }
                    c.b = false;
                    a = false;
                    return true;
                } else
                {
                    com.mdotm.android.e.d.b(this, (new StringBuilder("loading other url ")).append(s).toString());
                    return super.shouldOverrideUrlLoading(webview, s);
                }
            }

            
            {
                c = MdotMActivity.this;
                super();
                b = new _cls1(this);
            }
        });
        d = (b)getIntent().getExtras().getSerializable("response");
        f = getIntent().getExtras().getLong("InterstitialActionListenerId");
        com.mdotm.android.e.d.b(this, (new StringBuilder("listener id ")).append(f).toString());
        e = com.mdotm.android.e.f.a().a(f);
        if (d == null) goto _L2; else goto _L1
_L1:
        if (getIntent().getExtras().getInt("ScreenOrientation") != 1) goto _L4; else goto _L3
_L3:
        setRequestedOrientation(7);
_L6:
        k = d.g();
        RelativeLayout relativelayout = new RelativeLayout(this);
        android.widget.RelativeLayout.LayoutParams layoutparams = new android.widget.RelativeLayout.LayoutParams(-1, -1);
        relativelayout.setLayoutParams(layoutparams);
        relativelayout.setBackgroundColor(0xff444444);
        if (o)
        {
            relativelayout.setBackgroundColor(0);
            RelativeLayout relativelayout1 = new RelativeLayout(this);
            new android.widget.RelativeLayout.LayoutParams(-2, -2);
            relativelayout1.setLayoutParams(layoutparams);
            relativelayout1.setBackgroundResource(p);
            relativelayout.addView(relativelayout1);
            if (android.os.Build.VERSION.SDK_INT > 11)
            {
                relativelayout.setAlpha(1.0F);
            }
        }
        if (1 == n && !o && android.os.Build.VERSION.SDK_INT > 11)
        {
            relativelayout.setAlpha(0.9F);
        }
        com.mdotm.android.e.d.b(this, (new StringBuilder("resource type ")).append(k).toString());
        if (k == com.mdotm.android.b.c.f && d.b() == f.b)
        {
            h = new InterstitialImageView(this, d, this);
            if (!h.a)
            {
                b();
                com.mdotm.android.e.d.a(this, "Failed to load interstitial banner ad");
            } else
            {
                m = h();
                relativelayout.addView(h);
                relativelayout.addView(h());
                com.mdotm.android.e.d.b(this, "Call report impression imageresource");
                com.mdotm.android.e.f.a().a(d.c(), this);
            }
        } else
        if (k == com.mdotm.android.b.c.h || d.g() == com.mdotm.android.b.c.i || d.g() == com.mdotm.android.b.c.f && d.b() == f.a)
        {
            com.mdotm.android.e.d.b(this, "loading html page");
            i = new com.mdotm.android.view.a(this, d, this, n);
            i.setGravity(13);
            relativelayout.addView(i);
            m = h();
            relativelayout.addView(m);
        } else
        if (k == com.mdotm.android.b.c.g)
        {
            g = new com.mdotm.android.view.c(this, d, this);
            relativelayout.addView(g());
        } else
        {
            com.mdotm.android.e.d.a(this, (new StringBuilder("This format is not supported ")).append(d.d()).toString());
            onDismissScreen();
        }
        setContentView(relativelayout);
        return;
_L4:
        if (getIntent().getExtras().getInt("ScreenOrientation") == 2)
        {
            setRequestedOrientation(6);
        }
        continue; /* Loop/switch isn't completed */
_L2:
        com.mdotm.android.e.d.b(this, "Ad response is null");
        onDismissScreen();
        return;
        if (true) goto _L6; else goto _L5
_L5:
    }

    protected void onDestroy()
    {
        super.onDestroy();
        com.mdotm.android.e.f.a().b(f);
        if (g != null && a != null)
        {
            g.a(a);
        }
    }

    public void onDismissScreen()
    {
        com.mdotm.android.e.d.b(this, "onDismiss is called");
        c = true;
        if (e != null)
        {
            e.a();
        } else
        {
            com.mdotm.android.e.d.a(this, "InterstitialActionListener is null. So ignoring the interstitial dismiss callback");
        }
        finish();
    }

    public void onFailedToReceiveBannerAd()
    {
    }

    public void onFailedToReceiveInterstitialAd()
    {
    }

    public void onInterstitialAdClick()
    {
    }

    public void onInterstitialDismiss()
    {
    }

    public boolean onKeyDown(int i1, KeyEvent keyevent)
    {
        com.mdotm.android.e.d.b(this, (new StringBuilder("Key down ")).append(i1).toString());
        if (i1 == 4)
        {
            onDismissScreen();
        }
        return super.onKeyDown(i1, keyevent);
    }

    public void onLeaveApplicationFromBanner()
    {
    }

    public void onLeaveApplicationFromInterstitial()
    {
    }

    protected void onPause()
    {
        com.mdotm.android.e.d.b(this, "onPause is called");
        super.onPause();
        if (g != null)
        {
            g.a();
        }
        if (!l && !c)
        {
            onDismissScreen();
        }
    }

    public void onReceiveBannerAd()
    {
    }

    public void onReceiveInterstitialAd()
    {
    }

    protected void onResume()
    {
        com.mdotm.android.e.d.b(this, "onResume is called");
        if (!l) goto _L2; else goto _L1
_L1:
        onDismissScreen();
        l = false;
_L4:
        super.onResume();
        return;
_L2:
        if (g != null && g.c())
        {
            g.b();
        }
        if (true) goto _L4; else goto _L3
_L3:
    }

    public void willShowInterstitial()
    {
    }

    // Unreferenced inner class com/mdotm/android/view/MdotMActivity$1$1

/* anonymous class */
    class _cls1
        implements Runnable
    {

        final _cls1 a;

        public void run()
        {
            if (a.a)
            {
                com.mdotm.android.e.d.a(this, "timeout loading landing url");
                com.mdotm.android.view.MdotMActivity.a(com.mdotm.android.view._cls1.a(a)).stopLoading();
                com.mdotm.android.view._cls1.a(a).b = false;
                a.a = false;
            }
        }

            
            {
                a = _pcls1;
                super();
            }
    }

}
