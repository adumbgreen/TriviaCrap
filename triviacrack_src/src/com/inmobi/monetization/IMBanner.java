...
public final class IMBanner extends RelativeLayout
{
    ...
    private void a()
    {
        ...
        if (InternalSDKUtil.isInitializedSuccessfully())
        {
            ...
            {
                Log.internal("[InMobi]-[Network]-4.3.0", "Cannot start ice. Activity is null");
            }
            if (e != -1L)
            {
                b();
    ...
    private void a(int i1)
    {
        ...
        if (i1 < j1)
        {
            Log.debug("[InMobi]-[Network]-4.3.0", (new StringBuilder()).append("Refresh Interval cannot be less than ").append(j1).append(" seconds. Setting refresh rate to ").append(j1).toString());
        }
        h();
    }
        ...
        if (a != null)
        {
            ...
            } else
            {
                    ...
                    implements Runnable
                {
                    ...
                    public void run()
                    {
                        Log.debug("[InMobi]-[Network]-4.3.0", "AppId Not Set - Please call setAppId on the IMBanner object with a valid appId. For XML, please add \"appId\" attribute to the XML. Please refer integration guideline for more details.");
...
_L3:
    ...
    public void setAppId(String s1)
    {
        ...
        {
            Log.debug("[InMobi]-[Network]-4.3.0", "AppId cannot be null or blank.");
            return;
        }
        if (e != -1L)
        {
            Log.debug("[InMobi]-[Network]-4.3.0", "A new appId cannot be set if a slotId is already set.");
            return;
        } else
        {
            q = s1;
    ...
    public void setKeywords(String s1)
    {
        if (s1 == null || "".equals(s1.trim()))
        {
            Log.debug("[InMobi]-[Network]-4.3.0", "Keywords cannot be null or blank.");
            return;
        } else
        {
            m = s1;
    ...
    public void setRefTagParam(String s1, String s2)
    {
        if (s1 == null || "".equals(s1.trim()))
        {
            Log.debug("[InMobi]-[Network]-4.3.0", "Ref tag key cannot be null or blank.");
            return;
        }
        if (s2 == null || "".equals(s2.trim()))
        {
            Log.debug("[InMobi]-[Network]-4.3.0", "RefTag value cannot be null or blank.");
            return;
        } else
        {
            n = s1;
    ...
    public void setRequestParams(Map map)
    {
        ...
        {
            Log.debug("[InMobi]-[Network]-4.3.0", "Request params cannot be null or empty.");
            return;
        } else
        {
            p = map;
    ...
    public void setSlotId(long l1)
    {
        if (l1 == -1L)
        {
            Log.debug("[InMobi]-[Network]-4.3.0", "Invalid slotId : -1");
        } else
        {
            if (e == -1L && q != null)
            {
                Log.debug("[InMobi]-[Network]-4.3.0", "A new slotId can be set only if a valid slotId is provided during object construction.");
                return;
            }
            e = l1;
            if (a != null)
        ...
        public void handleMessage(Message message)
        {
            ...
            if (imbanner != null)
            {
                ...
                if (IMBanner.e(imbanner) != -1)
                {
                    ...
                    {
                        Log.debug("[InMobi]-[Network]-4.3.0", "Activity is not in the foreground. New ad will not be loaded.");
                        return;
                    }
                    if (MRAIDInterstitialController.isInterstitialDisplayed.get())
                    {
                        Log.debug("[InMobi]-[Network]-4.3.0", "Ad cannot be loaded.Interstitial Ad is displayed.");
                        return;
                    } else
                    {
