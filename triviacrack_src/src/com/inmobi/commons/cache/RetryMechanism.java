    ...
    private void a(RetryRunnable retryrunnable, int i)
    {
        class a extends TimerTask
        {
            ...
            public void run()
            {
                ...
                {
                    Log.internal("[InMobi]-4.3.0", "Exception occured while running retry mechanism and will the limit for retrying has been reached.");
                    a.completed();
                    return;
                } else
                {
                    Log.internal("[InMobi]-4.3.0", (new StringBuilder()).append("Exception occured while running retry mechanism and will retry in ").append(RetryMechanism.b(b) * RetryMechanism.d(b)).append(" ms").toString());
