...
_L1:
    ...
    public String toPayloadString(List list, List list1, Context context)
    {
        ...
        try
        {
            ...
            jsonobject.put("mk-version", (new StringBuilder()).append("pr-SAND-").append(InternalSDKUtil.getInMobiInternalVersion("4.3.0")).append("-").append("20140428").toString());
            s = currentLocationStr();
        }
        catch (JSONException jsonexception)
        {
            Log.internal("[InMobi]-4.3.0", "Unable to create payload for sending ThinICE params");
            return null;
        }
        if (s == null)
        {
