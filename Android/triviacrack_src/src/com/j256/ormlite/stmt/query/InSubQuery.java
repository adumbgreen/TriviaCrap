...
public class InSubQuery extends BaseComparison
{
    ...
    public void appendValue(DatabaseType databasetype, StringBuilder stringbuilder, List list)
    {
        ...
        if (afieldtype != null)
        {
            ...
            {
                throw new SQLException((new StringBuilder()).append("There must be only 1 result column in sub-query but we found ").append(afieldtype.length).toString());
            }
            if (fieldType.getSqlType() != afieldtype[0].getSqlType())
            {
                throw new SQLException((new StringBuilder()).append("Outer column ").append(fieldType).append(" is not the same type as inner column ").append(afieldtype[0]).toString());
            }
        }
        stringbuilder.append(") ");
    }
