...
//            SISRegisterEventRequest, SISUpdateDeviceInfoRequest, Configuration
...
label0:
        {
            ...
            if (!exceededCheckinInterval(l) && !registrationinfo.shouldGetNewSISDeviceIdentifer() && !registrationinfo.shouldGetNewSISRegistration())
            {
                boolean flag1 = DebugProperties.getDebugPropertyAsBoolean("debug.shouldRegisterSIS", false);
                flag = false;
                if (!flag1)
                {
                    break label0;
    ...
    {
        SISDeviceRequest sisdevicerequest = (new SISGenerateDIDRequest()).setAdvertisingIdentifierInfo(info);
        (new SISRequestor(new RegisterEventsSISRequestorCallback(), new SISRequest[] {
            sisdevicerequest
        })).startCallSIS();
    }
...
label0:
    ...
    {
        SISDeviceRequest sisdevicerequest = (new SISUpdateDeviceInfoRequest()).setAdvertisingIdentifierInfo(info);
        (new SISRequestor(new RegisterEventsSISRequestorCallback(), new SISRequest[] {
            sisdevicerequest
        })).startCallSIS();
    }
    ...
    private class RegisterEventsSISRequestorCallback
        implements SISRequestorCallback
    {
        ...
        public RegisterEventsSISRequestorCallback()
        {
            sisRegistration = SISRegistration.this;
        }
    }
