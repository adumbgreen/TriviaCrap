...
public final class AnalyticsNetworkManager
{
    ...
    private AnalyticsNetworkManager()
    {
        e = 0;
        (new a()).start();
        Log.debug("[InMobi]-[Analytics]-4.3.0", "NetworkManager-> Constructor ");
    }
    ...
    private String a(String s, String s1, boolean flag)
    {
        ...
        s4 = (new StringBuilder()).append("pr-SAND-").append(InternalSDKUtil.getInMobiInternalVersion("4.3.0")).append("-").append("20140428").toString();
    ...
    private void a(AnalyticsPayload analyticspayload, String s)
    {
        String s1;
        int i;
        Log.internal("[InMobi]-[Analytics]-4.3.0", (new StringBuilder()).append("Ping URL: ").append(AnalyticsUtils.getEventUrl()).toString());
        s1 = analyticspayload.getCompletePayload();
        i = AnalyticsInitializer.getConfigParams().getMinEventsToCompress();
        Log.internal("[InMobi]-[Analytics]-4.3.0", (new StringBuilder()).append("Post body: ").append(s1).toString());
        if (analyticspayload.getPayloadSize() < i || android.os.Build.VERSION.SDK_INT < 8) goto _L2; else goto _L1
_L1:
        Log.debug("[InMobi]-[Analytics]-4.3.0", "Compressing the payload");
        boolean flag = true;
...
_L5:
        ...
        Log.debug("[InMobi]-[Analytics]-4.3.0", (new StringBuilder()).append("Post body: ").append(s2).toString());
        ...
        int k = l;
_L6:
        Log.debug("[InMobi]-[Analytics]-4.3.0", (new StringBuilder()).append("Http Status Code: ").append(k).toString());
        if (k != 200)
        {
...
_L3:
        ...
        try
        {
            ...
            Log.debug("[InMobi]-[Analytics]-4.3.0", (new StringBuilder()).append("Ping Response: ").append(s4).toString());
        ...
        catch (Exception exception1)
        {
            try
            {
                Log.internal("[InMobi]-[Analytics]-4.3.0", "Unable to read from stream");
                return;
            }
            // Misplaced declaration of an exception variable
            catch (Exception exception)
            {
                Log.debug("[InMobi]-[Analytics]-4.3.0", "Exception Pinging", exception);
            }
        }
        e = 1 + e;
        if (e >= AnalyticsUtils.getMaxRetryBeforeDiscard())
...
_L2:
    ...
    // Unreferenced inner class com/inmobi/commons/analytics/net/a
    final class com.inmobi.commons.analytics.net.a
...
label0:
                    {
                        Log.debug("[InMobi]-[Analytics]-4.3.0", (new StringBuilder()).append("NetworkManager->handleMessag: msg:").append(message).toString());
