...
public class AnalyticsInitializer
{
    ...
    private static void a(Context context)
    {
        if (context != null && a == null)
        {
            if (a == null)
            {
                ...
                {
                    Log.internal("[InMobi]-[Analytics]-4.3.0", (new StringBuilder()).append("Exception while retreiving configs due to commons Exception with code ").append(commonsexception.getCode()).toString());
                    return;
                }
                catch (Exception exception)
                {
                    Log.internal("[InMobi]-[Analytics]-4.3.0", "Exception while retreiving configs.", exception);
                    return;
                }
            }
        } else
    ...
    private static boolean b(Map map)
    {
        ...
        {
            Log.internal("[InMobi]-[Analytics]-4.3.0", "Exception while saving configs.", exception);
            return false;
        }
        return true;
    }
    ...
    // Unreferenced inner class com/inmobi/commons/analytics/bootstrapper/a
    final class a
