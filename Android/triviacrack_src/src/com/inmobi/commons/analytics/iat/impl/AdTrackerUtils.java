...
public class AdTrackerUtils
{
    ...
    public static boolean checkDownloadGoalAdded()
    {
        ...
        {
            Log.internal("[InMobi]-[AdTracker]-4.3.0", "Application Context NULL cannot checkStatusUpload");
            return false;
        } else
        {
    ...
    public static String getReferrerFromLogs()
    {
        ...
        Log.internal("[InMobi]-[AdTracker]-4.3.0", "Getting referrer from logs");
...
_L1:
        ...
        if (matcher.find())
        {
          ...
          goto _L1
        Log.internal("[InMobi]-[AdTracker]-4.3.0", (new StringBuilder()).append("Received referrer from logs: ").append(s).toString());
        return s;
        Exception exception;
        exception;
...
_L3:
        Log.internal("[InMobi]-[AdTracker]-4.3.0", "Getting referrer from logs failed", exception1);
        return s;
        exception1;
...
_L2:
    ...
    public static boolean isPermissionGranted(String s)
    {
        ...
        Exception exception;
        exception;
        Log.internal("[InMobi]-[AdTracker]-4.3.0", "Cant check permissions", exception);
        return false;
    }
    ...
    public static void reportMetric(AdTrackerEventType adtrackereventtype, Goal goal, int i, long l, int j, String s)
    {
        ...
        {
            Log.internal("[InMobi]-[AdTracker]-4.3.0", "Error reporting metric", exception);
            return;
        }
    ...
    public static boolean sendBroadcastMessage(int i)
    {
        ...
        {
            Log.internal("[InMobi]-[AdTracker]-4.3.0", "Cant send test broadcast", exception);
            return false;
        }
        return true;
    }
    ...
    public static void setInternalReferrer(Context context, String s)
    {
        ...
        if (context == null || s == null)
        {
            try
            {
                Log.internal("[InMobi]-[AdTracker]-4.3.0", "Cannot set Market Referrer..Referrer NULL");
                return;
            }
            catch (Exception exception)
            {
                Log.internal("[InMobi]-[AdTracker]-4.3.0", "Cannot set referrer", exception);
            }
        ...
        Log.internal("[InMobi]-[AdTracker]-4.3.0", (new StringBuilder()).append("Saving referrer from broadcast receiver: ").append(s).toString());
        ...
        Log.debug("[InMobi]-[AdTracker]-4.3.0", "Initialization incomplete. Please call InMobi initialize with a valid app Id");
        return;
        return;
    ...
    public static void setReferrerFromLogs(Context context, String s)
    {
        if (context == null || s == null)
        {
            try
            {
                Log.internal("[InMobi]-[AdTracker]-4.3.0", "Cannot set Market Referrer from logs..Referrer NULL");
                return;
            }
            catch (Exception exception)
            {
                Log.internal("[InMobi]-[AdTracker]-4.3.0", "Cannot set referrer from logs", exception);
            }
