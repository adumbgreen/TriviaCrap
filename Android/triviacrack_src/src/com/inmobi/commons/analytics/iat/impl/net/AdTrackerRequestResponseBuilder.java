...
public class AdTrackerRequestResponseBuilder
{
    ...
    protected static String formHTTPRequest(String s, String s1, int i, boolean flag, String s2)
    {
        ...
        linkedlist.add(new BasicNameValuePair("u-id-map", AdTrackerInitializer.getConfigParams().getUID().getUidMap(1, s4, null)));
        ...
        s5 = (new StringBuilder()).append("pr-SAND-").append(InternalSDKUtil.getInMobiInternalVersion("4.3.0")).append("-").append("20140428").toString();
        linkedlist.add(new BasicNameValuePair("mk-version", s5));
        linkedlist.add(new BasicNameValuePair("mk-rel-version", s5));
        linkedlist.add(new BasicNameValuePair("osV", String.valueOf(android.os.Build.VERSION.SDK_INT)));
        s6 = FileOperations.getPreferences(InternalSDKUtil.getContext(), "IMAdTrackerStatusUpload", "rlc");
        if (s6 == null)
        {
            s6 = "0";
        }
        linkedlist.add(new BasicNameValuePair("rlc", s6));
        try
        {
            s7 = InternalSDKUtil.getContext().getPackageManager().getPackageInfo(InternalSDKUtil.getContext().getPackageName(), 0).versionName;
        }
        catch (android.content.pm.PackageManager.NameNotFoundException namenotfoundexception)
        {
            Log.internal("[InMobi]-[AdTracker]-4.3.0", "Cant get appversion", namenotfoundexception);
            s7 = null;
        }
        if (s7 != null && !s7.trim().equals(""))
        {
            linkedlist.add(new BasicNameValuePair("u-appver", s7));
        }
    ...
    public static com.inmobi.commons.analytics.iat.impl.AdTrackerConstants.StatusCode sendHTTPRequest(String s, Goal goal, String s1)
    {
        ...
        String s6;
        int j;
        Log.debug("[InMobi]-[AdTracker]-4.3.0", s4);
        ...
        Log.internal("[InMobi]-[AdTracker]-4.3.0", (new StringBuilder()).append("RESPONSE: ").append(s5).toString());
...
_L4:
        ...
        Log.debug("[InMobi]-[AdTracker]-4.3.0", (new StringBuilder()).append("Error uploading ping ").append(s6).append("\nReloading webview").toString());
...
_L7:
        ...
        Log.internal("[InMobi]-[AdTracker]-4.3.0", "Error uploading Goal Ping", exception1);
...
_L5:
    ...
    public static boolean serverReachable(String s)
    {
        ...
        Exception exception;
        exception;
        Log.debug("[InMobi]-[AdTracker]-4.3.0", "Server not reachable..Logging events");
        return false;
    }
