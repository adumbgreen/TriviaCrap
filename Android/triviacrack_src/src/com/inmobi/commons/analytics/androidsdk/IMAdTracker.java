...
public final class IMAdTracker
{
    ...
    private boolean a(String s)
    {
        ...
        Log.debug("[InMobi]-[AdTracker]-4.3.0", "Please call init() with valid context and app id");
        return false;
        if (s == null)
        {
        ...
        Log.debug("[InMobi]-[AdTracker]-4.3.0", "Please pass a valid GoalName");
        return false;
        ...
        if ("download".equals(s) && flag)
        {
            try
            {
                Log.debug("[InMobi]-[AdTracker]-4.3.0", "Download goal already uploaded");
            }
            catch (Exception exception)
            {
                Log.internal("[InMobi]-[AdTracker]-4.3.0", "Cannot report goal", exception);
                return false;
            }
            return false;
        }
        ...
        if (!AdTrackerUtils.checkDownloadGoalAdded())
        {
            ...
            Log.debug("[InMobi]-[AdTracker]-4.3.0", (new StringBuilder()).append("Goal Queued ").append(s).toString());
...
_L1:
    ...
    public void init(Context context, String s)
    {
        if (context == null)
        {
            try
            {
                Log.debug("[InMobi]-[AdTracker]-4.3.0", "Application Context NULL");
                Log.debug("[InMobi]-[AdTracker]-4.3.0", "context cannot be null");
                return;
            }
            catch (Exception exception)
            {
                Log.internal("[InMobi]-[AdTracker]-4.3.0", "Failed to init", exception);
            }
        ...
        Log.debug("[InMobi]-[AdTracker]-4.3.0", "APP ID Cannot be NULL");
        Log.debug("[InMobi]-[AdTracker]-4.3.0", "appId cannot be null");
        return;
        return;
        if (s.trim().equals(""))
        {
            Log.debug("[InMobi]-[AdTracker]-4.3.0", "appId cannot be blank");
            return;
        }
        Log.debug("[InMobi]-[AdTracker]-4.3.0", "IMAdtracker init successfull");
        ...
        {
            Log.debug("[InMobi]-[AdTracker]-4.3.0", "Add android.permission.INTERNET and android.permission.ACCESS_NETWORK_STATE permission in Android manifest");
            return;
        }
        if (android.os.Build.VERSION.SDK_INT < 17 && !AdTrackerUtils.isPermissionGranted("android.permission.READ_LOGS"))
        {
            Log.debug("[InMobi]-[AdTracker]-4.3.0", "Add android.permission.READ_LOGS permission in Android Manifest");
        }
    ...
    public void reportCustomGoal(String s)
    {
        if ("download".equals(s))
        {
            Log.debug("[InMobi]-[AdTracker]-4.3.0", "Download Goal should be reported using reportAppDownloadGoal()..");
            return;
        } else
        {
            a(s);
